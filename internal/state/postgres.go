...
func (s *PostgresStore) ClaimProcessed(ctx context.Context, assignmentID, dedupKey string) (bool, error) {
	// ON CONFLICT DO NOTHING makes the claim atomic across concurrent fires:
	// exactly one INSERT affects a row; the loser sees RowsAffected() == 0.
...
func (s *PostgresStore) UnmarkProcessed(ctx context.Context, assignmentID, dedupKey string) error {
	_, err := s.db.Exec(ctx,
		"DELETE FROM assignment_processed WHERE assignment_id=$1 AND dedup_key=$2",
		assignmentID, dedupKey)
	return err
}
