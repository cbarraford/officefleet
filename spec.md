...
- **Surfaces:**
  ...
  2. **Agents management** — create/edit/delete agents; set role, system prompt, default LLM; **assign
     duties** (pick from the Duty library and configure each for this agent: trigger, outputs, config
     values, backend override, and per-agent prompt tweaks — extra instructions or a task-prompt
     override). "Assignment" stays invisible — the screen reads as *"this agent's duties."*
  ...
  4. **Per-agent detail pages** — the agent's duties, run history (event→system+task prompt→result→
     outputs), stats, and per-assignment state/memory.
...
- **SP4 — Web UI & operators.** API + SPA (all surfaces: dashboard, Agents, Duty library, per-agent
  pages, integrations/settings), auth + Admin/Viewer roles, encrypted secrets, live dashboard via SSE.
